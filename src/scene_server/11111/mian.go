...
// 补全自定义层级的bk_biz_id字段
func addBizIDOnSelfDefinedNode() error {

	mongoConf := local.MongoConf{}
	mongoConf.TimeoutSeconds = 20
	mongoConf.MaxOpenConns = 100
	mongoConf.MaxIdleConns = 200
	mongoConf.RsName = "rs0"
	mongoConf.SocketTimeout = 30

	user := "bk_cmdb"
	password := "ca1a7c5d-4564-4377-9ac2-30b9ae11a939"
	address := "10.0.247.97:27017"
	database := "cmdb"
	mechanism := "SCRAM-SHA-1"
	mongoConf.URI = fmt.Sprintf("mongodb://%s:%s@%s/%s?authMechanism=%s", user, password, address, database, mechanism)
